...
def kmeans(list_features,centroids,k,iter_num):
    ...
    for label in set(predicted_labels):
        ...
        predicted_labels = [common_label if predicted_labels[i] == label else predicted_labels[i] for i in range(len(predicted_labels))]
...
def readCSV(file_name): 
    ...
    for line in open(data_file):
        ...
        [list_features[i].append(fields[i]) if i == num_cols - 1 else list_features[i].append(float(fields[i])) for i in range(len(fields))]
